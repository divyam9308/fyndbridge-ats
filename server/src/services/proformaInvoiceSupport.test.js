...
test('creation chooser, URL-backed switcher, global tax KPIs, and proforma columns are wired', () => {
  ...
  assert.match(invoicePage, /const BILLING_TOTAL_ROWS = \[\.\.\.BILLING_ENTITIES, COMBINED_BILLING_ENTITY\]/)
  assert.match(invoicePage, /BILLING_ENTITIES\.reduce\(\(sum, billingEntity\) => sum \+ BigInt/)
  assert.match(invoicePage, /BILLING_TOTAL_ROWS\.map\(billingEntity/)
