...
test('every report endpoint is mounted behind authentication and fresh Report page permission', () => {
  ...
  for (const endpoint of [
    ...
    "router.get('/consultant/export-preview'",
...
test('consultant options and direct report scopes are derived from authenticated server access', () => {
  ...
  assert.match(reportService, /async function getConsultantReportExport\(user, query\)[\s\S]*?parseReportRequest\(query, 'main'\)[\s\S]*?reportAccess\(user, params\.consultantUserId\)/)
...
test('Candidates report renders one responsive daily total series for individual and Overall Consultants data', () => {
  ...
  assert.match(reportPage, /Candidates Uploaded Each Date \(Total\)/)
  assert.match(reportPage, /<AreaChart data=\{dailyData\}/)
  assert.match(reportPage, /dataKey="candidateCount"/)
  assert.match(reportPage, /name="Candidates Uploaded"/)
  assert.match(reportPage, /fill="url\(#dailyCandidateAreaGradient\)"/)
  assert.match(reportPage, /ticks=\{weeklyTicks\}/)
  ...
  assert.match(reportPage, /label=\{\{ value: 'Candidates', angle: -90/)
  ...
  assert.match(reportStyles, /\.report-daily-chart\s*\{[\s\S]*?height:\s*320px/)
  assert.match(reportStyles, /\.report-chart-tooltip\s*\{[\s\S]*?background:\s*#171719/)
})
