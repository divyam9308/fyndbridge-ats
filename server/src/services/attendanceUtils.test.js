...
const {localDate,workedMinutes,calculateLeave,applySandwichContext,getFinancialYearForDate,getFinancialYearRange,getFinancialYearMonths,getLeaveAccrualSchedule,calculateLeaveEntitlement,calculateCarryForward}=require('./attendanceUtils')
...
test('sandwich context requires leave on both Saturday and Monday',()=>{
  const saturdayOnly=applySandwichContext(calculateLeave({startDate:'2026-07-11',endDate:'2026-07-11',durationType:'full_day',balance:6}),['2026-07-11'])
  const mondayOnly=applySandwichContext(calculateLeave({startDate:'2026-07-13',endDate:'2026-07-13',durationType:'full_day',balance:6}),['2026-07-13'])
  const bothSides=applySandwichContext(calculateLeave({startDate:'2026-07-11',endDate:'2026-07-11',durationType:'full_day',balance:6}),['2026-07-11','2026-07-13'])
  assert.equal(saturdayOnly.charged_leave_days,1)
  assert.equal(mondayOnly.charged_leave_days,1)
  assert.equal(bothSides.charged_leave_days,2)
  assert.equal(bothSides.calculation_breakdown.find(item=>item.date==='2026-07-12')?.reason,'Sandwich leave')
})
