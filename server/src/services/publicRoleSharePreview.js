...
function publicExperienceLabel(value) {
  const experience = clean(value)
  if (!experience) return 'Experience not specified'
  return /\by(?:ea)?rs?\b/i.test(experience) ? experience : `${experience} Years`
}
...
function buildPublicRoleShareHtml(role, frontendOrigin) {
  ...
  const imageUrl = `${origin}/share/open-roles/${slug}/image.jpg`
...
<html lang="en">
  <head>
    ...
    <meta property="og:image:alt" content="${escapeHtml(`${roleName} — ${location}, ${experience}`)}" />
