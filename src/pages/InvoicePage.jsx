...
function CreateInvoiceModal({ entities, onClose, onCreated }) {
  ...
  return createPortal(<div className="modal-overlay"><div className="modal-card modal-card-lg invoice-modal invoice-generate-modal" role="dialog" aria-modal="true">
    ...
    <div className="modal-body">{error && <div className="invoice-form-error">{error}</div>}
      <section className="invoice-form-section"><h3>Entity</h3><select className="form-control" value={selectedId} onChange={select}><option value="">Select billing entity</option>{entities.map(entity => <option key={entity.id} value={entity.id}>{entity.entity_display_id || entity.invoice_id} - {display(entity.legal_entity_name)}</option>)}</select>{selected && <div className="invoice-selected-chip"><Check size={14} />{display(selected.legal_entity_name)}</div>}</section>
...
export default function InvoicePage() {
  ...
  return <div className="invoice-page">
    <div className="candidate-page-header"><div><h2>Invoice</h2><p>Manage billing entities and their invoices.</p></div><div className="header-actions"><button className="btn-secondary" onClick={() => setCreating(true)}><FileText size={15} />Create Invoice</button><button className="btn-primary" onClick={() => setAdding(true)}><Plus size={15} />Add Entity</button></div></div>
