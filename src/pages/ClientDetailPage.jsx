...
export default function ClientDetailPage() {
  ...
  const renderCandidateCell = ({ key }, c) => {
    switch (key) {
      ...
      case 'cv': return <td key={key}>{resolveCandidateCvHref(c) ? <a href={resolveCandidateCvHref(c)} target="_blank" rel="noopener noreferrer" className="cv-table-link" title="Open CV" onClick={() => logCandidateCvOpen(c)}><FileText size={15} /></a> : '-'}</td>
