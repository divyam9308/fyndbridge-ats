...
import { ChevronLeft, FileText, LoaderCircle, Pencil, X } from 'lucide-react'
import { fetchInvoiceEntity, regenerateInvoice } from '../services/invoiceApi'
...
function EditInvoiceModal({ invoice, onClose, onSaved }) {
  ...
  const regenerate = async () => {
    ...
    try { await regenerateInvoice(invoice.id, form); await onSaved(); onClose() } catch (err) { setError(err.message); setSaving(false) }
  }
  ...
  return createPortal(<div className="modal-overlay"><div className="modal-card modal-card-lg invoice-modal invoice-generate-modal" role="dialog" aria-modal="true">
    ...
    </div></section><section className="invoice-form-section"><h3>Updated Total</h3><div className="invoice-preview"><span>Taxable<b>{money(calc.taxable)}</b></span><span>Grand Total<b>{money(calc.grand)}</b></span></div></section></div>
    <div className="modal-footer"><button className="btn-secondary" onClick={onClose}>Cancel</button><button className="btn-primary" onClick={regenerate} disabled={saving}>{saving ? 'Regenerating...' : 'Save & Regenerate'}</button></div>
...
export default function InvoiceEntityDetailPage() {
  ...
  return <div className="invoice-page invoice-entity-details">
    ...
    <div className="table-card invoice-table-card"><div className="invoice-card-toolbar"><strong>Generated Invoices</strong><span>{data.invoices.length} invoices</span></div><div className="table-scroll"><table className="data-table invoice-detail-table"><thead><tr>{['Invoice ID', 'Consultant Name', 'Candidate Name', 'Calculation Details', 'Invoice', 'Actions'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>
      {data.invoices.map(invoice => <tr key={invoice.id}><td><span className="invoice-id">{invoice.invoice_display_id || invoice.invoice_number}</span></td><td>{show(invoice.consultant_name)}</td><td>{show(invoice.candidate_name)}</td><td>{details(invoice)}</td><td>{invoice.pdf_storage_path ? <button className="invoice-document-button" onClick={() => openInvoice(invoice)} title="Open invoice PDF">{opening === invoice.id ? <LoaderCircle size={16} /> : <FileText size={16} />}</button> : '-'}</td><td><button className="row-action-btn" onClick={() => setEditing(invoice)} aria-label="Edit invoice"><Pencil size={14} /></button></td></tr>)}
