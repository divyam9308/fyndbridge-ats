...
function EditInvoiceModal({ invoice, entity, entities, onClose, onSaved }) {
  ...
  const saveRegenerated = async () => {
    setSaving(true); setError('')
    try { const result = await regenerateInvoice(invoice.id, { ...form, invoice_type: invoice.invoice_type }); setSaved(result); await onSaved() } catch (err) { setError(err.message) } finally { setSaving(false) }
  }
