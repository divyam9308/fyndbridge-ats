-- Tax invoices retain one sequence per billing entity. Proforma invoices use
-- one shared sequence across all billing entities for each financial year.
...
create unique index if not exists invoices_proforma_invoice_sequence_key
  on public.invoices (financial_year, sequence_number)
...
as $$
  ...
  from generate_series(
    ...
    coalesce((
      ...
      where invoice.financial_year = p_financial_year
        and invoice.invoice_type = p_invoice_type
        and (
          p_invoice_type = 'proforma_invoice'
          or invoice.billing_entity = p_billing_entity
        )
    ), 0) + 1
